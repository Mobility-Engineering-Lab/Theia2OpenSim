...
Run everything with defaults (dynamic `.mot`, static `.trc` from `sample_data/c3d_trials/Static.c3d`,
...
| `--static-c3d`           | `sample_data/c3d_trials/Static.c3d`            | Dedicated static C3D used for the TRC, the pelvis reference pose, and scaling.                                                                                   |
...
| `--static-frames`        | `None` (runtime fallback: `"300"`)           | Frame(s) for the static pose, e.g.`"300"`, `"290:310"`, `"290,300,310"`.                                                                                   |
...
| `--output-osim`          | `sample_data/OpenSim_output/scaled_model.osim` | Output scaled`.osim` model path.                                                                                                                               |
...
Scaling only runs when a static source resolves (`--static-c3d` or `--static-frames`, and `--no-scale` is not passed) and `opensim-cmd` can be found; otherwise scaling is
